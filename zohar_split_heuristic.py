...
def split_letters_new(tgt_doc, src_doc, langs, max_words, atomic_line):
...
def split_and_save(tgt_doc, src_doc, langs, words_threshold, atomic_line, tgt_out, src_out):
    ...
    with open(tgt_doc, encoding='utf-8') as tgt, open(src_doc, encoding='utf-8') as src:
        res = split_letters_new(tgt.read(), src.read(), langs, words_threshold, atomic_line)
...
def main():
    ...
    split_and_save(args.target_file, args.source_file, (args.source, args.target),
                   args.words_threshold, args.atomic_line, tgt_output, src_output)
